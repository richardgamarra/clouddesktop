  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ maxHeight: 180, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2, paddingRight: 2 }}>
        {groups.map(g => (
          <div key={g.label}>
            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text3)', fontFamily: "'DM Mono',monospace", padding: '4px 8px', textTransform: 'uppercase', letterSpacing: '.06em' }}>
              {g.label}
            </div>
            {g.channels.map(ch => {
              const isActive = ch.id === active
              return (
                <button key={ch.id} onClick={() => { setActive(ch.id); setError(false); const origin = encodeURIComponent(location.origin); if (iframeRef.current) iframeRef.current.src = `https://www.youtube-nocookie.com/embed/${ch.ytId}?autoplay=1&rel=0&enablejsapi=1&origin=${origin}` }}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '7px 10px',
                    borderRadius: 7, cursor: 'pointer', textAlign: 'left', border: 'none',
                  ...
                  {isActive && <span style={{ fontSize: 8, color: 'var(--green)' }}>● LIVE</span>}
                </button>
              )
            })}
          </div>
