...
const WIDGET_TYPES = [
  ...
  { type: 'stocks',     icon: '📈',  name: 'Stocks',        desc: 'Stock prices via proxy',           defaultConfig: { symbols: ['AAPL','TSLA','MSFT','GOOGL','AMZN'] } },
...
export default function WidgetsTab({ tab, onUpdateTab }) {
  ...
  function addWidget(type) {
    ...
    const newWidget = { id: 'w_' + Date.now(), type, config: { ...def.defaultConfig } }
    onUpdateTab(tab.id, { config: { ...tab.config, widgets: [...widgets, newWidget] } })
    setShowPicker(false)
  }
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:20 }}>
        ...
        {widgets.length > 0 && (
          <span style={{ fontSize:11, color:'var(--text3)', fontFamily:"'DM Mono',monospace" }}>{widgets.length} widget{widgets.length !== 1 ? 's' : ''}</span>
        )}
      </div>
            ...
            <div key={widget.id} style={cardStyle}>
              ...
              <div style={headerStyle}>
                ...
                <button onClick={() => removeWidget(widget.id)}
                  style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', fontSize:16, lineHeight:1, padding:'2px 4px', position:'absolute', top:12, right:12 }}
